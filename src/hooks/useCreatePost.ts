...
export function useCreatePost(): () => Promise<void> {
  ...
  const createPost = async (): Promise<void> => {
    ...
    if (currentMenu === Action.ENCRYPT_PASTEBIN) {
      const res = await encrypt(text, passkey);
      const newNewlink = await postPastebin(res.data, api_key);
      const history = {
      ...
      const newNewlink = await postPastebin(text, api_key);
      const history = {
        action: Action.ENCRYPT_PASTEBIN,
        id: uuidv4(),
        pastebinlink: newNewlink,
      ...
      if (passkey !== '') {
        ...
        if (pasteText) {
          const res = decrypt(pasteText, passkey);
          dispatch({
      ...
      if (passkey !== '') {
        const res = decrypt(text, passkey);
        dispatch({
          type: Action.UPDATE_PLAINTEXT,
          payload: {
            plaintext: res,
