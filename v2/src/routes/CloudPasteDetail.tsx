...
import { Globe, EyeOff, Lock, Loader2, ExternalLink, FileText, Trash2, Copy, Check } from 'lucide-react'
import { useStore } from '@/lib/store'
import { getPastebin, deletePastebin, extractPasteKey, type PasteItem } from '@/lib/pastebin'
...
export default function CloudPasteDetail() {
  ...
  const handleDelete = async () => {
    ...
    setDeleteError('')
    try {
      await deletePastebin(settings.apiKey, settings.userKey, extractPasteKey(paste.url))
      navigate(-1)
    } catch (e) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 pb-4">
            ...
            <div className="flex items-center gap-3">
              {content && (
                <button
                  ...
                  className={cn('flex items-center gap-1 text-xs transition-colors', urlCopied ? 'text-success' : 'text-text-muted hover:text-text-secondary')}
                >
          ...
          <div className="rounded-xl border border-border bg-surface-secondary/50 overflow-hidden min-h-[6rem]">
            {loadingContent && (
            ...
            {content !== null && !loadingContent && (
              <textarea
                ...
                value={content}
                rows={10}
                className="w-full px-3 py-2.5 text-[12px] font-mono bg-transparent resize-none focus:outline-none text-text-primary leading-relaxed"
              />
            )}
          </div>
        </div>
